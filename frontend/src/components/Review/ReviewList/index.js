...
export default function ReviewList({ avgStarRating, numReviews, Owner, id:spotId }) {
    ...
    <div>
      ...
      {user && Owner && (user.id !== Owner.id) && !userHasSpotReviewCheck() && <CreateAReviewButton spotId={spotId}/>}
      {reviewsArray.length > 0 ?
      <div className="list-of-reviews">
            {reviewsArray.map((review)=>{
                return <SingleReview key={review.id} reviewId={review.id} User={review.User} createdAt={review.createdAt} review={review.review}/>
            })}
